...
from mathLib import mm, subtractVList, subtractVectors, crossProduct, normV, inverse
...
class Renderer(object):
    ...
    def glTriangle_bc(self, A, B, C, verts = (), texCoords = (), normals = (), clr = None):
        ...
        edge1 = subtractVectors(verts[1], verts[0])
        edge2 = subtractVectors(verts[2], verts[0])

        triangleNormal = crossProduct(edge1, edge2)
        triangleNormal = normV(triangleNormal)

        deltaUV1 = subtractVList(texCoords[1], texCoords[0])
        deltaUV2 = subtractVList(texCoords[2], texCoords[0])
        
        op = (deltaUV1[0]* deltaUV2[1] - deltaUV2[0] * deltaUV1[1])
        f = 0
        if op != 0:
            f = 1 / op
        ...
        tangent = normV(tangent)

        bitangent = crossProduct(triangleNormal, tangent)
        bitangent = normV(bitangent)
